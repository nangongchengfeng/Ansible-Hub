...
async def test_business_node_permission_routes_exist():
    ...
    async with AsyncClient(
        ...
        response = await client.get("/business-nodes/1/permissions")
        assert response.status_code in [200, 401, 403, 404]

        # Test setting permissions (should return 401)
        response = await client.put("/business-nodes/1/permissions", json={"permissions": []})
        assert response.status_code in [200, 401, 403, 404, 422]

        # Test binding gateway (should return 401)
        response = await client.put("/business-nodes/1/gateway", json={"gateway_id": None})
        assert response.status_code in [200, 401, 403, 404, 422]

        # Test getting hosts (should return 401)
        response = await client.get("/business-nodes/1/hosts")
        assert response.status_code in [200, 401, 403, 404]
